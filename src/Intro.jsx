...
function Intro() {
  ...
  useEffect(() => {
    // Checking dark modes pwerensence
    const darkModeMediaQuery = window.matchMedia("(prefers-color-scheme: dark)");

    // setting the state as pre the theme
  ...
  return (
    <>
      <div className={`absolute top-0 left-0 w-full h-full z-[-1] ${isDarkMode ? "" : "hidden"}`}>
        <video
          src="/bg1.mp4" // Reference the video correctly
          autoPlay
          loop
          muted
      ...
      <div className={`h-screen w-screen flex flex-col items-center justify-center p-4 ${isDarkMode ? "" : "bg-blue-700"}`}>
